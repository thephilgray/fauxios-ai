...
`;
    ...
    let imagePrompt = `A satirical news image related to a satirical article with the headline: "${parsedSections["Headline"]}". The image should be absurdly humorous in a subtle way, without text or logos, suitable for a publication that mixes historical analysis with modern news.`;

    if (articleHeadline.toLowerCase().includes('president') || articleContent.toLowerCase().includes('president')) {
      imagePrompt += ` The image should depict the US president in the style of a political cartoon of King George III from the American Revolutionary era. He should have a yellow wig instead of a white one, be wearing a royal red coat, and be adorned with an excessive amount of gold.`
    }
